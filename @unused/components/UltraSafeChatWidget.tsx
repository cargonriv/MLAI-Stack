...
interface UltraSafeChatWidgetProps {
  isOpen: boolean;
  onToggle: () => void;
}

const UltraSafeChatWidget = ({ isOpen, onToggle }: UltraSafeChatWidgetProps) => {
  ...
  // Safe tokenization function that never crashes
  const safeGetTokenCount = useCallback((text: string): number => {
    ...
    try {
      ...
      // Additional safety: check if tokenizer has encode method
      ...
      // Disable tokenizer on error to prevent future crashes
      setTokenizerError(true);
      return 0;
    }
  }, [tokenizer, tokenizerError]);

  // Initialize tokenizer with maximum safety
  useEffect(() => {
    const initTokenizer = async () => {
      // Prevent multiple initializations
      if (initializationRef.current || tokenizer || tokenizerError) {
        return;
      }
      ...
      try {
        ...
        const { AutoTokenizer } = await Promise.race([importPromise, timeoutPromise]) as any;
        
        // Load tokenizer with timeout
        const tokenizerPromise = AutoTokenizer.from_pretrained('Xenova/gpt-4o');
        const tokenizerTimeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Tokenizer load timeout')), 30000)
        );
        
        const tok = await Promise.race([tokenizerPromise, tokenizerTimeoutPromise]);
        ...
        console.log('Tokenizer test result:', testResult2);
        
        if (!Array.isArray(testResult2) || testResult2.length !== 2) {
          console.warn('Unexpected tokenizer result, but continuing...');
        }
        
        // Test 4: Edge cases
        const testResult3 = tok.encode('');
        const testResult4 = tok.encode(' ');
        ...
        // Add welcome message only after successful initialization
    ...
    // Only initialize when chat is opened and not already initialized
    if (isOpen && !initialized && !isLoadingTokenizer) {
  ...
  }, [isOpen, initialized, isLoadingTokenizer, tokenizer, tokenizerError, messages.length]);
  ...
  const generateResponse = async (userMessage: string): Promise<string> => {
    ...
    if (lowerMessage.includes('tokenizer') || lowerMessage.includes('gpt-4o') || lowerMessage.includes('token')) {
      const tokenCount = safeGetTokenCount(userMessage);
      const tokenizerStatus = tokenizer && !tokenizerError ? 'active' : 'fallback';
      
      return `Great question about tokenization! ${tokenizerStatus === 'active' ? "I'm using the GPT-4o tokenizer to process our conversation." : "I'm running in fallback mode, but I can still explain tokenization!"} ${tokenizerStatus === 'active' && tokenCount > 0 ? `For example, your message "${userMessage}" was tokenized into ${tokenCount} tokens.` : ""}\n\nTokenization is crucial for:\n• **Text preprocessing** for ML models\n• **Context management** in conversations\n• **Efficient processing** of natural language\n\nThe GPT-4o tokenizer is state-of-the-art for understanding text structure!`;
    }
    ...
    const responses = [
      ...
      `Interesting! ${tokenizer && !tokenizerError ? `I processed your message using ${safeGetTokenCount(userMessage)} tokens with the GPT-4o tokenizer.` : "I'm running in fallback mode but can still help!"} While I focus on Carlos's portfolio, I'm happy to discuss how the ML models and technologies here work. What would you like to explore?`
    ];
  ...
  const handleSendMessage = async () => {
    ...
    // Create user message with safe token counting
    const userTokens = safeGetTokenCount(trimmedInput);
    const userMessage: Message = {
      id: Date.now().toString(),
      content: trimmedInput,
      sender: 'user',
      timestamp: new Date(),
      tokens: userTokens > 0 ? userTokens : undefined
    };
    ...
    try {
      ...
      const botTokens = safeGetTokenCount(response);
      const botMessage: Message = {
        id: (Date.now() + 1).toString(),
        content: response,
        sender: 'bot',
        timestamp: new Date(),
        tokens: botTokens > 0 ? botTokens : undefined
      };
  ...
  // Safe input token display
  const getInputTokenDisplay = () => {
    if (!inputValue?.trim() || !tokenizer || tokenizerError) {
      return null;
    }
    
    try {
      const count = safeGetTokenCount(inputValue);
      return count > 0 ? `${count} tokens` : null;
    } catch {
      return null;
    }
  };
        ...
        {!isMinimized && (
          <div className="flex items-center gap-2 text-sm opacity-90">
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
              ...
              {tokenizer && !tokenizerError ? 'GPT-4o Tokenizer' : 'Fallback Mode'}
            </Badge>
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
      ...
      {!isMinimized && (
        <CardContent className="p-0 flex flex-col h-[calc(100%-80px)]">
          ...
          <div className="p-4 border-t">
            ...
            {(() => {
              const tokenDisplay = getInputTokenDisplay();
              return tokenDisplay ? (
...
export default UltraSafeChatWidget;