...
interface NoTokenizerChatWidgetProps {
  isOpen: boolean;
  onToggle: () => void;
}

const NoTokenizerChatWidget = ({ isOpen, onToggle }: NoTokenizerChatWidgetProps) => {
  ...
  // Initialize tokenizer ONLY after component is fully mounted and chat is opened
  useEffect(() => {
  ...
  // Safe token counting - only called when explicitly needed and tokenizer is ready
  const getTokenCount = (text: string): number => {
    if (tokenizerStatus !== 'ready' || !tokenizerRef.current || !text) return 0;
    try {
      return tokenizerRef.current.encode(text).length;
    } catch (error) {
  ...
  const generateResponse = async (userMessage: string): Promise<string> => {
    ...
    if (lowerMessage.includes('tokenizer') || lowerMessage.includes('gpt-4o') || lowerMessage.includes('token')) {
      ...
      const statusText = tokenizerStatus === 'ready' ? "I'm using the GPT-4o tokenizer to process our conversation." : 
                        tokenizerStatus === 'loading' ? "I'm still loading the GPT-4o tokenizer." :
                        "I'm running in fallback mode, but I can still explain tokenization!";
      
      return `Great question about tokenization! ${statusText} ${tokenizerStatus === 'ready' && tokenCount > 0 ? `For example, your message "${userMessage}" was tokenized into ${tokenCount} tokens.` : ""}\n\nTokenization is crucial for:\n• **Text preprocessing** for ML models\n• **Context management** in conversations\n• **Efficient processing** of natural language\n\nThe GPT-4o tokenizer is state-of-the-art for understanding text structure!`;
    }
    ...
    const responses = [
      ...
      `Interesting! ${tokenizerStatus === 'ready' ? `I processed your message using ${getTokenCount(userMessage)} tokens with the GPT-4o tokenizer.` : "I'm still loading the tokenizer but can help!"} While I focus on Carlos's portfolio, I'm happy to discuss how the ML models and technologies here work. What would you like to explore?`
    ];
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage: Message = {
      ...
      sender: 'user',
      timestamp: new Date(),
      tokens: tokenizerStatus === 'ready' ? getTokenCount(trimmedInput) : undefined
    };
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        sender: 'bot',
        timestamp: new Date(),
        tokens: tokenizerStatus === 'ready' ? getTokenCount(response) : undefined
      };
        ...
        {!isMinimized && (
          <div className="flex items-center gap-2 text-sm opacity-90">
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
              ...
              {tokenizerStatus === 'ready' ? 'GPT-4o Ready' : 
               tokenizerStatus === 'loading' ? 'Loading...' : 
               tokenizerStatus === 'error' ? 'Fallback Mode' : 'Starting...'}
            </Badge>
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
...
export default NoTokenizerChatWidget;