...
interface SafeChatWidgetProps {
  isOpen: boolean;
  onToggle: () => void;
}

const SafeChatWidget = ({ isOpen, onToggle }: SafeChatWidgetProps) => {
  ...
  useEffect(() => {
    const initTokenizer = async () => {
      ...
      try {
        ...
        // Add fallback message without tokenizer
    ...
    if (isOpen && !isLoadingTokenizer && messages.length === 0) {
      initTokenizer();
    }
  }, [isOpen, tokenizer, messages.length]);
  ...
  const generateResponse = async (userMessage: string): Promise<string> => {
    ...
    if (lowerMessage.includes('tokenizer') || lowerMessage.includes('gpt-4o') || lowerMessage.includes('token')) {
      const tokenCount = tokenizer ? getTokenCount(userMessage) : 'N/A';
      return `Great question about tokenization! ${tokenizer ? "I'm using the GPT-4o tokenizer to process our conversation." : "I'm running in fallback mode, but I can still explain tokenization!"} ${tokenizer ? `For example, your message "${userMessage}" was tokenized into ${tokenCount} tokens.` : ""}\n\nTokenization is crucial for:\n• **Text preprocessing** for ML models\n• **Context management** in conversations\n• **Efficient processing** of natural language\n\nThe GPT-4o tokenizer is state-of-the-art for understanding text structure!`;
    }
    ...
    const responses = [
      ...
      `Interesting! ${tokenizer ? `I processed your message using ${getTokenCount(userMessage)} tokens with the GPT-4o tokenizer.` : "I'm running in fallback mode but can still help!"} While I focus on Carlos's portfolio, I'm happy to discuss how the ML models and technologies here work. What would you like to explore?`
    ];
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage: Message = {
      ...
      sender: 'user',
      timestamp: new Date(),
      tokens: tokenizer ? getTokenCount(trimmedInput) : undefined
    };
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        sender: 'bot',
        timestamp: new Date(),
        tokens: tokenizer ? getTokenCount(response) : undefined
      };
        ...
        {!isMinimized && (
          <div className="flex items-center gap-2 text-sm opacity-90">
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
              ...
              {tokenizer ? 'GPT-4o Tokenizer' : 'Fallback Mode'}
            </Badge>
            <Badge variant="secondary" className="text-xs bg-white/20 text-white border-white/30">
